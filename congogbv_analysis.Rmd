...
sample_grouped <-
  sample %>%
  mutate(group = complete_ra(N = nrow(.),conditions = groups))
...
predictions_forest <-
  bind_cols(data_forest_selected %>% select(KEY,vill_id, ball5),
           tibble(predict_rf = forest_selected %>% predict()))
...
compare_targeting(c("reg","rf","conf"), 15)
