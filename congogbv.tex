\input{congogbv_preamble}
\input{congogbv_customfunctions}
...
\todo{Erwin: 1208: Add paper of Erwin and Robert}
...
%is nu een beetje dubbel op misschien?
\todo{Erwin: 1208: je moet ergens een duidelijke onderzoeksvraag geven. Of in jouw geval: misschien twee onderzoeksvragen.  Staat er nu nog niet...}
...
\section*{Methods}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\todo{Erwin 1208: conflict data: over welke periode heb je conflict gegevens verzameld (survey, ACLED)? Hoe verhoud deze window zich tot de burgeroorlog? Dit bespreek je pas bij de resultaten (p13) maar kan iets uitgebreider en naar voren.}
...
\input{C:/Users/Koen/Dropbox/PhD/Papers/CongoGBV/Tables/balance.tex}
...
\todo{Erwin 1208 -p13: Veronderstel dat het bovenstaande klopt (2012 geweldvragen stellen je in staat iets te zeggen over indirecte effecten, in 2014 ACLED data stellen je in staat iets te zeggen over zowel directe en indirecte effecten). Als geweld door strijders een rol speelt, dan zou de tweede analyse dus sterkere effecten moeten laten zien. Je vindt het omgekeerde. Blijkbaar zijn directe effecten niet sterk. Maar waarom gelden de indirecte effecten (nog) niet? Met andere woorden: als je concludeert dat directe effecten zwak zijn; zou je dan niet hetzelfde resultaten verwachten te vinden voor de 2012 survey data en de 2014 ACLED data (namelijk: alleen indirecte effecten). Wat is hier aan de hand denk je?
}
\todo{Erwin 1208 -p13: is het aannemelijk dat huishoudens (of dorpen) die voor 2012 zijn aangevallen in 2013 alweer zijn aangevallen? Met andere woorden; is er sprake van targeting/selectie? In dat geval loopt het effect dat je vindt misschien niet via veranderende normen, maar heeft geweld exposure in 2012 voorspelende waarde voor opnieuw geweld exposure in 2013... ?}
\begin{figure}[htb]
  \includegraphics[width=\linewidth]{C:/Users/Koen/Dropbox/PhD/Papers/CongoGBV/Figures/meancompare_conf.png}
  \caption{Comparison of means of issues faced across conflict exposure.}
  \label{fig:meancompare_conf}
\end{figure}

The first dimension over which I create sub-groups is conflict exposure. I use two different data sources for conflict. First, for \summstat{victimproplost}{nall} households, detailed conflict data is available from the 2012 round of data collection. Because my indicator for for SGBV victimization here is limited to the 12 months before the interview in 2014, any relationship found between these conflict events, and incidence of SGBV cannot be due to perpetration by armed groups during the conflict event. Secondly, I use data from the ACLED as a source for more recent conflict exposure. 
...
\input{"C:/Users/Koen/Dropbox/PhD/Papers/CongoGBV/Tables/conflict_by_terr"}
\end{table}
...
When looking at more recent exposure to conflict, no clear patterns emerge (see bottom panel of Figure \ref{fig:meancompare_conf}). SGBV incidence among women who have more instances of violence near them than the median is 7 percentage points lower than women who do not (bottom row of Table \ref{tab:meandiff_conf}). However, this is not statistically significant. The fact that conflict before 2012 correlates with SGBV, but recent conflict does not, points at a more complex relationship between conflict and SGBV than a simple direct effect due to perpetration by armed groups. \todo{Erwin 1208: a more complex relationship... prima.  Maar wat dan?}

\begin{table}[htb]
\caption{Differences in numbers of issued faced in the list experiment, across conflict indicators}
\label{tab:meandiff_conf}
\meandifftab{"C:/Users/Koen/Dropbox/PhD/Papers/CongoGBV/Tables/meandifftab_conf.csv"}
\end{table}

\subsection*{Intra-household bargaining position}
\todo{Erwin 1280: de relatie tussen intrahousehold bargaining en conflict is niet duidelijk. Is het ook zo dat conflict invloed heeft op bargaining positie van vrouwen (als gemeten in het spelletje)? Zou je kunnen bespreken in een voetnoot.}
\todo{Erwin 1280: Misschien moet je in de inleiding iets aanpassen, en duidelijk maken dat je data je in staat stelt iets te zeggen over SGBV door strijdende partijen en door partners, en het relatieve belang van deze groepen “daders”.  Dit gaat dus (weer) over de onderzoeksvragen.}
\begin{figure}[htb]
  \includegraphics[width=\linewidth]{C:/Users/Koen/Dropbox/PhD/Papers/CongoGBV/Figures/meancompare_mar.png}
...
The finding that conflict history is associated with an increase in SGBV, while recent conflict is not, points to the indirect relationship between conflict and SGBV, where conflict may affect SGBV rates not through perpetration by armed groups, but by an increase in IPV. This notion that IPV is a major driver of SGBV is reinforced by the fact that both intra-household bargaining position and secondary education are negatively associated with SGBV.  This suggests that the position of women is important in protecting them from human rights violations.
...
\section*{Conclusion}
\todo{Erwin 1208: de conclusie is duidelijk, hier wordt mij pas duidelijk wat je echt beoogd hebt met de analyse... Ik vind de belangrijkste conclusie interessant en ook wel credible. Een focus op rape als een weapon of war is veel te beperkt. Maar la je data had kunnen verzamelen vlak na een episode van veel geweld, dan had je misschien andere dingen gevonden. Veel SGBV door strijdende partijen vind denk ik plaats in korte hevige episodes. In het “overall plaatje”  zijn strijders misschien toch belangrijk? Of denk je dat je ACLED data van 2013-2014 een goed en representatief beeld geven van geweld tijdens de burgeroorlog in de DRC?}
...
\bibliography{C:/Users/Koen/Dropbox/Literatuur/Mendeley/Bibtex/Thesis-CongoGBV}
