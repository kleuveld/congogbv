...
\end{figure}
I then explore the relationship between gender attitudes of both partners, and the incidence of SGBV, based on the proposition presented in Table \ref{tab:genderatt}. I recode the answers so that 5 is always fully agree with the empowered proposition, and 1 is fully agree with the less empowered option. I then sum all the answers to obtain a measure of gender attitudes. On average, the mean score for men was  \summstat{atthusbtotal}{meanall}. Women's attitudes were comparatively empowered: the mean score for them was \summstat{attwifetotal}{meanall}. To create sub-groups, I then split the respondents in groups with higher or lower values than the median.

The number of issues faced by each sub-group are displayed in Figure \ref{fig:meancompare_att}. The number of issues face are similar across sub-groups. In Table \ref{tab:meadifftab_att} these differences are displayed. Though incidence of SGVC is slightly lower for both empowered male respondents, as empowered female respondents, these differences are not statistically significant.
