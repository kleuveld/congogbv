...
\csvstyle{coeftable}{tabular=lccccccc,table head= \toprule & \multicolumn{3}{l}{Group 0} &  \multicolumn{3}{l}{Group 1} & DiD \\ Variable & Control & Treatment & Diff & Control & Treatment & Diff &  \\\toprule,late after line=\\,table foot=\bottomrule}

\newcommand{\meandifftab}[1]{
	\csvreader[coeftable]{#1}%
	{varlabel=\var,meancontrol0 = \mca, meantreat0 = \mta ,stardiff0=\diffa, sediff0=\sea, meancontrol1 = \mcb, meantreat1 = \mtb , stardiff1=\diffb, sediff1=\seb, stardd = \dd, sedd=\sedd}%
	{\var\ & \mca & \mta & \diffa & \mcb & \mtb & \diffb & \dd \\ & & & (\sea) & & & (\seb) & (\sedd) }
...
The third conflict indicator I examine, is the number of cases of violence against civilians in a 30km radius in the past month. I split the sample in two, using the median as a cutoff point. See \ref{fig:meancompare_conf3}. In the non-victimized group (n=\incid{acledviolence30d0}{n}), the average number of issues was \incid{acledviolence30d0}{mean0} for the control group, and \incid{acledviolence30d0}{mean1} for the treatment group. This implies a rate of SGBV of \incid{acledviolence30d0}{incidence_pct}\% (p = \incid{acledviolence30d0}{p}). For the group that has suffered loss of property due to the conflict (n=\incid{acledviolence30d1}{n}), the average number of issues was \incid{acledviolence30d1}{mean0} for the control group, and \incid{acledviolence30d1}{mean1} for the treatment group. This implies a rate of SGBV of \incid{acledviolence30d1}{incidence_pct}\% (p = \incid{acledviolence30d1}{p}). Note that the results presented here are robust to using number of battles or number of fatalities rather than the instances of violence against civilians; using 5,10,15,20 or 25km as a radius, or using a continuous variable, rather than a binary variable.  
...
\paragraph{}
As above, I statistically test the differences in the differences of means by inserting the variables examined above in equation \ref{eq:interaction}. The results are displayed in Table \ref{tab:meandiff_conf}. None of the coefficients plotted are significant at the 95\% level.

\begin{table}[h]
\caption{Differences in numbers of issued faced in the list experiment, across conflict indicators}
\begin{threeparttable}
\meandifftab{"C:/Users/Koen/Dropbox (Personal)/PhD/Papers/CongoGBV/Tables/conflict_regs.csv"}
\label{tab:meandiff_conf}
\begin{tablenotes}
\small
 \item Standard errors in parentheses. * p $<$ 0.1, ** p $<$ 0.05 *** p $<$ 0.01
 \item Group assigmment is as follows: for property lost and household member killed, group 1 contains the households which have lost property or family members. For the ACLED data, group 1 contains those households with more civilians killed within 10kms of their house than the median household. Group 0 is always the reference group.
 \item The diff columns provide estimates of the incidence of SGBV in each group; the DiD columns an estimate of the difference between the groups.
\end{tablenotes}
\end{threeparttable}
\end{table}
...
\small
 \item Standard errors in parentheses. * p $<$ 0.1, ** p $<$ 0.05 *** p $<$ 0.01
 \item The diff columns provide estimates of the incidence of SGBV in each group; the DiD columns an estimate of the difference between the groups.
 \item Group 0 contains the households for which the variable contains ``no'', group 1 contains the households for which it is ``yes''.
\end{tablenotes}
\end{threeparttable}
\end{table}
