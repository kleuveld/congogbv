...
plot1 <- 
  ...
  filter(var == "victimproplost") %>%
  ggplot(aes(numballs, group, colour = Treatment)) +
    geom_pointrange(aes(xmin = ci.lower, xmax = ci.higher), size = 0.1, position = position_dodge(width=0.5) ) +
    xlim(0,5) + 
    xlab("Number of reported issues") +
    ylab("Conflict pre-2012: property lost") +
    theme(text = element_text(size=8))

plot2 <- 
  read_csv(here("tables/dataforgraphs.csv")) %>%
  filter(var == "victimfamlost") %>%
  ggplot(aes(numballs, group, colour = Treatment)) +
    geom_pointrange(aes(xmin = ci.lower, xmax = ci.higher), size = 0.1, position = position_dodge(width=0.5) ) +
    xlim(0,5) + 
    xlab("Number of reported issues") +
    ylab("Conflict pre-2012:\nHH member killed") +
    theme(text = element_text(size=8))


plot3 <- 
  read_csv(here("tables/dataforgraphs.csv")) %>%
  filter(var == "acledviolence10d") %>%
  ggplot(aes(numballs, group, colour = Treatment)) +
    geom_pointrange(aes(xmin = ci.lower, xmax = ci.higher), size = 0.1, position = position_dodge(width=0.5) ) +
    xlim(0,5) + 
    xlab("Number of reported issues") +
    ylab("Conflict 2013−2014: Instances \n of violence against civilians") +
    theme(text = element_text(size=8))
...
plot1 <- 
  ...
  ggplot(aes(numballs, group, colour = Treatment)) +
    ...
    xlim(0,5) + 
    xlab("Number of reported issues") +
    ylab("Land holdings of families before marriage") +
    theme(text = element_text(size=8))

plot2 <- 
  read_csv(here("tables/dataforgraphs.csv")) %>%
  filter(var == "bargresult") %>%
  ggplot(aes(numballs, group, colour = Treatment)) +
    geom_pointrange(aes(xmin = ci.lower, xmax = ci.higher), size = 0.1, position = position_dodge(width=0.5) ) +
    xlim(0,5) + 
    xlab("Number of reported issues") +
    ylab("Bargaining result") +
    theme(text = element_text(size=8))
