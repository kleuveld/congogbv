...
summstats %>%
  ...
  set_caption(caption = "Descriptive statistics by treatment assignment", 
              style = "Table Caption", 
              autonum = run_autonum(seq_id = "tab", bkm = "summstats")) %>%
  ...
  #for whatever reason, the table footer won't render nicely; i hope i can get to it later

  # add_footer_lines(values = "*p < 0.1,**p < 0.05,***p < 0.01", top = FALSE) %>%
  # fontsize(size = 3, part = "footer") %>%
  # autofit() %>%
  # FitFlextableToPage() 
